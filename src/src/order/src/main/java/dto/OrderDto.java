...
public class OrderDto {
    ...
    public String getTabSeparatedString(){
        return this.getName()+"\t"+this.getQuantity();
    }

    public static OrderDto fromTSVString(String orderDetails){
        try{
            String[] orderDetailsList = orderDetails.split("\t");
            return new OrderDto(orderDetailsList[0],Integer.parseInt(orderDetailsList[1]));
        }catch (Exception e){
            return null;
        }
    }
